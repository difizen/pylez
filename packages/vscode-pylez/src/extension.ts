 ...
 *
 * Provides client for Pylez Python language server. This portion runs
...
export async function activate(context: ExtensionContext) {
    // See if Pylance is installed. If so, don't activate the Pylez extension.
    ...
    if (pylanceExtension) {
        window.showErrorMessage(
            'Pylez has detected that the Pylance extension is installed. ' +
                'Pylance includes the functionality of Pylez, and running both of ' +
                'these extensions can lead to problems. Pylez will disable itself. ' +
                'Uninstall or disable Pylez to avoid this message.'
        );
        return;
    }

    const pyrightExtension = extensions.getExtension('ms-python.vscode-pyright');
    if (pyrightExtension) {
        window.showErrorMessage(
            'Pylez has detected that the Pyright extension is installed. ' +
                'Pyright includes the functionality of Pylez, and running both of ' +
                'these extensions can lead to problems. Pylez will disable itself. ' +
                'Uninstall or disable Pylez to avoid this message.'
        );
        return;
    ...
    const clientOptions: LanguageClientOptions = {
        ...
        synchronize: {
            ...
            configurationSection: ['python', 'pylez'],
        },
    ...
    const client = new LanguageClient('python', 'Pylez', serverOptions, clientOptions);
    ...
    if (context.extensionMode === ExtensionMode.Development) {
        ...
        commands.executeCommand('setContext', 'pylez.development', true);
